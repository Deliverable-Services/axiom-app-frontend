...
const useProductVariables = (initialVariations) => {
    ...
    useEffect(() => {
        initialVariations
            ...
            .map(item => {
                const hasDefaultValue = item.items.findIndex((item) => item.default == 1);
                setSelectedVariations(prevValue => ({
                    ...prevValue,
                    [item.title]: hasDefaultValue ? item.items[hasDefaultValue] : item.items[0]
                }))
            })
    }, [initialVariations]);
    ...
    useEffect(() => {
        const formattedData = initialVariations 
            ...
            .filter(item => {
                ...
                if (selectedVariations && selectedVariations[relatedTo.title] && Object.keys(selectedVariations[relatedTo.title])) {
                    if (relatedItems.findIndex(item => item.id == selectedVariations[relatedTo.title]?.id) != -1) {
                        return true;
                    }
                } else {
                    ...
                    return {
                        ...
                        items: item.items
                                ...
                                .filter(option => {
                                    ...
                                    if (selectedVariations && selectedVariations[relatedTo.title] && Object.keys(selectedVariations[relatedTo.title])) {
                                        if (relatedItems.findIndex(relatedItem => relatedItem.id == selectedVariations[relatedTo.title].id) != -1) {
                                            return true;
                                        }
                                    } else {
                                        // - If the key is not present in selectedVariants then hide the value.
    ...
    const onVariantRadioChange = (evt, variantTitle, variantType) => {
        ...
        if (choosedVariant) {
          const choosedItem = choosedVariant?.items.find((item) => item.id == value);
    ...
    return { variations, selectedVariations, onVariantChange, onVariantRadioChange };
};
